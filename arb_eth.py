...
SUDDEN_DROP_TIMEFRAME = 3      # Check drop over 4 seconds
POSITION_SIZE = 6
MIN_ENTRY_LIQUIDITY = 100
...
HEDGE_STOP_LOSS_PRICE = 0.47  # If Lead Leg bid drops below this, trigger stop loss
...
MAX_HIGH_BID = 0.7   # Absolute max to avoid overpaying For "HIGH" preference
...
target_wait_time = 421 # The sell minute check 
MARKET_DURATION = 900
...
sustainment_time = 0.5  # Seconds the price must hold below trigger to confirm exit
...
class ETHArbitrageBot:
    def __init__(self):
        print("🤖 ETH Arbitrage Bot Starting...")
    ...
    def run(self):
        while True:
            ...
            m = self.get_market_from_slug(f"eth-updown-15m-{ts}")
