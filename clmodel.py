...
ML_CONFIDENCE_THRESHOLD = 0.84  # Only trade when confidence > 85%
...
TAKE_PROFIT_PERCENT = 0.20          # Exit at 20% gain from entry
STOP_LOSS_PERCENT = 0.40            # Stop loss at 40% loss from entry
TRAILING_STOP = True                # Use trailing stop
TRAILING_STOP_PERCENT = 0.15        # Trail by 15% from peak

# Market Sanity Checks
MIN_ENTRY_PRICE = 0.65              # Don't buy if price < $0.15 (too risky/likely wrong prediction)
MAX_ENTRY_PRICE = 0.87              # Don't buy if price > $0.80 (limited upside)
...
class MLPredictor:
    ...
    def can_predict(self):
        """Check if the market has reached the 7.5 minute mark"""
        # We need at least a few data points to prevent errors
        if len(self.current_market_data) < 5:
            return False
        ...
        current_market_age = df['time_elapsed'].max()

        # CRITICAL: If we just started the bot and the market is ALREADY 
        # past the 7.5 min mark, we missed the valid window.
        if current_market_age > ML_DATA_COLLECTION_TIME + 60: # allow 60s buffer
            print("   ⚠️ Market too old for valid prediction. Skipping.")
            return False
        
        # UPDATED: Check actual Market Age, not how long we've been watching
        current_market_age = df['time_elapsed'].max()
        
        # Trigger prediction as soon as market is older than 7.5 minutes
        if current_market_age >= ML_DATA_COLLECTION_TIME:
            return True
        
        # Emergency fallback: If market is ending soon (< 3 mins left)
        # and we have joined late, we should just predict now using what we have
        time_remaining = df['seconds_until_close'].iloc[-1]
        if time_remaining < 180 and len(df) > 20:
             print(f"   ⚠️ Emergency prediction: Market closing in {time_remaining}s")
             return True
    ...
    def _extract_features(self, df):
        """Extract features - MUST MATCH cltrain.py"""
        features = {}
        
        if len(df) == 0:
            return {k: 0 for k in self.feature_names}

        # Use 'yes_best_bid' from live data (corresponds to 'yes_price' in training)
        ...
        if features['yes_price_mean'] > 0:
             features['yes_volatility'] = features['yes_price_std'] / features['yes_price_mean']
...
class MLTradingBot:
    ...
    def execute_ml_strategy(self, market, market_start_time):
        ...
        # SANITY CHECK 1: Price too low (market doesn't believe this side will win)
        if entry_price < MIN_ENTRY_PRICE:
            market_favorite = "NO" if trade_side == 'YES' else "YES"
            market_favorite_price = snapshot['no_best_bid'] if trade_side == 'YES' else snapshot['yes_best_bid']
            print(f"\n⚠️ SKIPPING: {trade_side} price too low (${entry_price:.2f})")
            print(f"   Market strongly favors {market_favorite} @ ${market_favorite_price:.2f}")
            print(f"   Model may have inverted predictions - check training data!")
            self.traded_markets.add(slug)
            return "price_too_low"
        
        # SANITY CHECK 2: Price too high (limited upside)
        if entry_price > MAX_ENTRY_PRICE:
            print(f"⚠️ SKIPPING: Entry price too high (${entry_price:.2f})")
            print(f"   Limited upside potential")
            self.traded_markets.add(slug)
            return "price_too_high"
        
        # SANITY CHECK 3: Model vs Market disagreement
        market_says_yes = snapshot['yes_best_bid'] > snapshot['no_best_bid']
        model_says_yes = (trade_side == 'YES')
        
        if market_says_yes != model_says_yes:
            print(f"\n⚠️ WARNING: Model prediction conflicts with market pricing!")
            print(f"   Market believes: {'YES' if market_says_yes else 'NO'} will win")
            print(f"   Model predicts: {trade_side} will win ({prediction['confidence']:.1%})")
            print(f"   This suggests training data labels may be inverted!")
            # Don't skip, but warn user
        
        # CALCULATE POSITION SIZE: Fixed 5 Shares
        shares_to_buy = POSITION_SIZE  # Always uses 5 (from your config)
        
        max_cost_with_slippage = entry_price + ENTRY_SLIPPAGE
        max_cost = shares_to_buy * max_cost_with_slippage
        ...
        # Optional: Warn if 5 shares is too small for Polymarket ($1 min)
        # 5 shares @ $0.19 = $0.95 (This would fail API)
        if max_cost < 1.0:
             print(f"⚠️ WARNING: Order value ${max_cost:.2f} is below Polymarket $1 minimum.")
             # We proceed anyway as requested, but the API might reject it if price is very low (<0.20)
        ...
        print(f"Order Value: ${max_cost:.2f} (min ${MIN_ORDER_VALUE:.2f} required)")
        
        order_id, actual_shares = self.force_buy(token_to_buy, entry_price, shares_to_buy)
        ...
        print(f"   Total Cost: ${actual_shares * entry_price:.2f}")
