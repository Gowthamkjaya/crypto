...
# ==========================================
# Permutation 1: Mid-Game NO Lock (5-10 min remaining)
...
STOP_LOSS_DELAY = 300       # Wait 2 minutes before activating stop loss
...
class BTCMidGameBot:
    ...
    def execute_mid_game_lock(self, market, market_start_time):
        ...
        if time_remaining < NO_LOCK_WINDOW_START or time_remaining > NO_LOCK_WINDOW_END:
        ...
        # Determine which lock strategy to execute
        entry_token = None
        entry_side = None
        entry_price = None
        order_size = None
        
        # ONLY Permutation 1: NO Lock
        if no_price >= NO_LOCK_MIN_PRICE:
            entry_token = market['no_token']
            entry_side = "NO"
            entry_price = no_price
            order_size = NO_LOCK_ORDER_SIZE
            print(f"\n\n{'='*60}")
            print(f"🎯 MID-GAME NO LOCK TRIGGERED")
            print(f"{'='*60}")
        else:
            # NO price not high enough - no trade
    ...
    def run(self):
        ...
        print(f"📋 Strategy: Mid-Game NO Lock ONLY")
        print(f"   Entry: Buy NO @ ${NO_LOCK_MIN_PRICE:.2f}+ when 5-10min remaining")
        print(f"   Position size: {NO_LOCK_ORDER_SIZE} shares")
